...
<%@ page import="com.ly.crm.settings.domain.DicValue" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.ly.crm.workbench.domain.Tran" %>
...
<script type="text/javascript">
	...
	* */
	function changeStage(stage,i){
		...
		$.ajax({
			...
			data:{
				"id":"${t.id}",
				"stage":stage,
				"money":"${t.money}",	//生成交易历史使用
				"expectedDate":"${t.expectedDate}"	//生成交易历史使用
			},
			type: "post",
			dataType: "json",
			success:function (data){
	...
	function changeIcon(stage,i){
		...
		alert("当前阶段的下标" + index);
		alert("前面正常阶段和后面丢失解读那的分界点下标" +point);*/

		//如果当前阶段的可能性为0 前7个一定是黑圈，后两个一定是红叉，一个是黑叉
				...
				for (int i=0;i < dvList.size();i++){
					...
					if ("0".equals(listPossibility)){
						...
						if (i==index){
							...
							data-toggle="popover" data-placement="bottom"
							data-content="<%=dv.getText()%>" style="color: #90F780;"></span>
							-----------
							<%
							//如果小于当前阶段
						}else if (i < index){
